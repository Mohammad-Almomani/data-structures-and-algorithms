...
const screenForNames = (arr) => {
  return arr.filter(a => /^(Mr. |Mrs. |Ms. |Dr. )[A-Za-z]+$/gm.test(a))
}
...
let biggerThanLuke = (arr) => {
  let b = 77
  return arr.filter(a => {return (parseInt(a.mass)>parseInt(b))?a.name:'' }).map(star=>star.name).join(' - ')
};
...
const detectTicTacToeWin = (board) => {
  let a1 = [board[0][0],board[0][1],board[0][2]]
  let a2 = [board[1][0],board[1][1],board[1][2]]
  let a3 = [board[2][0],board[2][1],board[2][2]]

  let a4 = [board[0][0],board[1][0],board[2][0]]
  let a5 = [board[0][1],board[1][1],board[2][1]]
  let a6 = [board[0][2],board[1][2],board[2][2]]

  let a7 = [board[0][0],board[1][1],board[2][2]]
  let a8 = [board[0][2],board[1][1],board[0][0]]

  let test =[a1,a2,a3,a4,a5,a6,a7,a8]

    for (let i=0; i<test.length;i++){
        if(test[i][0]==test[i][1] && test[i][0] == test[i][2] && test[i][2]!=''){
            return true
        }
    }
    return false
