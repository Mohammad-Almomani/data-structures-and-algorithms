...
const countNumberOfChildren = (arr) => {
  let childNum =[];
  arr.reduce((acc,val) => val.hasOwnProperty('children')? childNum.push(val.children.length):childNum.push(0), {});
  return childNum.reduce((a,b)=>a+b)
};
...
const calculateAverage = (arr) => {
  let sum =0;
  return arr.reduce((a,b, idx)=> (sum+=b)/(idx+1),0)
};
...
const countPrimeNumbers = (arr) => {
  let count =0;
  arr.reduce((a,b)=> isPrime(b)?count+=1:count+=0 ,0)
  return count
};
...
const extractStat = (statName, arr) => {
  let correct ={};
  arr.reduce((a,b)=>{if (b.stat.name===statName) return correct = b},{});
  return correct;
};
...
const extractChildren = (arr) => {
  let childrens =[];
  arr.filter(a => a.name.includes('a')).reduce((a,b)=> {if (b.hasOwnProperty('children'))childrens.push(b.children)},{})
  return childrens.reduce((a,b)=>[...a,...b])
 };
