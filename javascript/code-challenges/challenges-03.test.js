...
const typeNum = (arr) => {
 return arr.filter(isNum => typeof isNum === 'number')
};
...
const containsAnd = (arr) => {
  return arr.filter (inclAnd => inclAnd.includes('and'))
};
...
const oddValues = (arr) => {
  return arr.filter(isOdd => isOdd%2===1 )
};
...
const extractChildren = (arr) => {
  let filtered = arr.filter(a => a.name.includes('a'))
  let childrens =[];
  filtered.reduce((a,b)=> {if (b.hasOwnProperty('children'))childrens.push(b.children)},{})
     return childrens
 };
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(st => {if (st.baseStat>minBaseStat) {
    return st;
  } 
}
  )
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(sta => {if (sta.baseStat>minBaseStat) {return sta}}).map (statName => statName.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(childCheck => !childCheck.children);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(isNum => typeof isNum === 'number').map(evenOdd => { return (evenOdd%2===0)?'even':'odd'});
};
