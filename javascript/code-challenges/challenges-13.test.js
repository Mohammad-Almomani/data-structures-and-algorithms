...
const standardizePhoneNumbers = (arr) => {
  return arr.map(num=> { return `${num.slice(1,4)}${num.slice(6,9)}${num.slice(10,14)}`})
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((str,idx) => idx%2==1).join('');
};
...
const allHappy = (arr) => {
 return arr.map(str=> str.includes(':)')).includes(false)?false:true;
};
